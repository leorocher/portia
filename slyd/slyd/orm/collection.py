...
class ModelCollection(OwnedList):
    ...
    def dump(self, state='working'):
        ...
        return self.model.file_schema(many=True, strict=True,
                                      context=context).dump(self).data
    ...
    def _set_related(self, related):
        if related and self.owner:
            ...
            for r in (r.with_snapshots(self.snapshots) for r in related):
                setattr(r, self.related_name, self.owner)
    ...
    def _clear_related(self, related):
        if related and self.owner:
            if not isinstance(related, list):
                related = [related]
            for r in (r.with_snapshots(self.snapshots) for r in related):
                if getattr(r, self.related_name, None) == self.owner:
                    setattr(r, self.related_name, None)
